import { EventCast, TCallback } from "src/utilities/TCallback";
import { useRideQueryHook } from "../utilityHooks";
import { RideAction } from "src/actions/actionTypes";
...
export const onRideStallCreate = (
  ...
  const rideCreateDemolishHook = context.subscribe("action.execute", (d) => {
    ...
    if (rideCreateCallback) {
      ...
      const timebetween =
        rideCreateDemolishQueue[0]?.timeStamp - rideCreateDemolishQueue[1]?.timeStamp;

      // start with if the most recent event was ridecreate
      if (action === "ridecreate")
        if (
          rideCreateDemolishQueue[1]?.action === "demolish" &&
          rideCreateDemolishQueue[0]?.ride === rideCreateDemolishQueue[1]?.ride &&
          timebetween < 10
        ) {
...
export const onRideStallDemolish = (
  ...
  const executeHook = context.subscribe<RideDemolishArgs>("action.execute", (d) => {
    const data = { ...d };
    if (
...
const addDataToQueue = (action: RideAction, data: GameActionEventArgs) => {
  const queueData = {
    ride: action === "ridecreate" ? (data.result as any).ride : (data.args as any).ride,
