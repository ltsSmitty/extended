...
export const onMazeSetTrack = (callback: TCallback): IDisposable =>
  context.subscribe("action.execute", (data: GameActionEventArgs<MazeSetTrackArgs>) => {
    if (data.action === "mazesettrack" && data.args.flags && data.args.flags < 0) {
      callback(data);
    }
  });
...
export const onMazePlaceTrack = (callback: TCallback): IDisposable =>
  context.subscribe("action.execute", (data: GameActionEventArgs<MazePlaceTrackArgs>) => {
    if (data.action === "mazeplacetrack" && data.args.flags && data.args.flags < 0) {
      callback(data);
    }
  });
